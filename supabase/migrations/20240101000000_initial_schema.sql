...
-- This is for reference, actual bucket creation should be done through Supabase
/*
INSERT INTO storage.buckets (id, name, public) VALUES ('winesnap-media', 'winesnap-media', true);

CREATE POLICY "Anyone can view public media" ON storage.objects FOR SELECT USING (bucket_id = 'winesnap-media');
CREATE POLICY "Authenticated users can upload media" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'winesnap-media' AND auth.role() = 'authenticated');
CREATE POLICY "Users can update their own media" ON storage.objects FOR UPDATE USING (bucket_id = 'winesnap-media' AND auth.uid()::text = (storage.foldername(name))[1]);
CREATE POLICY "Users can delete their own media" ON storage.objects FOR DELETE USING (bucket_id = 'winesnap-media' AND auth.uid()::text = (storage.foldername(name))[1]);
*/